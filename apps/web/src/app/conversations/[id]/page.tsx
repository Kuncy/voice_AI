...
function intakeTitle(call: DetailToolCall | undefined): string {
  if (!call) return "Conversation ohne Vorgang";
  if (call.damageReportId && call.damageCategory) return `Schadensmeldung ${categoryLabels[call.damageCategory]}`;
  if (call.serviceRequestType) return serviceRequestLabels[call.serviceRequestType];
  return "Strukturierter Vorgang";
}
  ...
  if (call.damageReportId) {
    return (
      <article className="intake-result-card intake-result-damage">
        <header>
          <span className="tool-kicker">ERGEBNIS · SCHADENSMELDUNG #{call.damageReportId.slice(0, 8)}</span>
        ...
        <dl>
          ...
          {call.damageUrgency && (
            <div>
              <dt>Dringlichkeit</dt>
              <dd className={`urgency urgency-${call.damageUrgency.toLowerCase()}`}>
                {urgencyLabels[call.damageUrgency]}
              </dd>
            </div>
          )}
        </dl>
    ...
    <article className="intake-result-card intake-result-service">
      <header>
        <span className="tool-kicker">
          ERGEBNIS · {serviceRequestLabels[call.serviceRequestType].toUpperCase()} #{call.serviceRequestId.slice(0, 8)}
        </span>
        <span className="status-pill status-succeeded">
          <i className="pill-dot" />
          {call.serviceStatus}
  ...
  return (
    <article className="tool-history-card">
      <div className="tool-history-summary">
        <time>{offset(call.createdAt, startedAt)}</time>
        <code>{call.toolName}</code>
        <span className={`status-pill status-${call.status.toLowerCase()}`}>
          <i className="pill-dot" />
          {call.status}
        </span>
        <span className="tool-duration numeric">{call.durationMs === null ? "–" : `${call.durationMs} ms`}</span>
      </div>
      ...
      <div className="conversation-detail-layout">
        <section className="conversation-detail-main">
          ...
          <div className="detail-heading-meta">
            <span>{conversation.createdAt.toLocaleString("de-DE")}</span>
            <span className={`status-pill status-${conversation.status.toLowerCase()}`}>
              <i className="pill-dot" />
              {conversation.status}
            </span>
          </div>
          <h1>{intakeTitle(intakeCalls[0])}</h1>
        ...
        <aside className="conversation-transcript">
          <div className="conversation-transcript-heading">
            <span>GESPRÄCHSVERLAUF</span>
            <small>{countLabel(messages.length, "Beitrag", "Beiträge")}</small>
          </div>
          {messages.length === 0 ? (
              ...
              return (
                <article className={`history-message history-message-${message.role.toLowerCase()}`} key={message.id}>
                  <div className="message-meta">
                    <strong>{roleLabels[message.role].toUpperCase()}</strong>
