...
export async function POST(request: NextRequest) {
  ...
  if (!consumeRateLimit("voice-reconnect", clientAddress(request), 10, 60_000)) {
    return NextResponse.json({ error: "Zu viele Verbindungsversuche. Bitte versuche es später erneut." }, { status: 429 });
  }

  try {
    const target = await new DrizzleConversationRepository(getWebDatabase().db).getReconnectTarget(handle.conversationId);
