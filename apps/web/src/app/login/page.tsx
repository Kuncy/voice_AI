  ...
  return (
    <main className="login-shell">
      <section className="login-card">
        ...
        <p className="settings-intro">Conversation-Verläufe, Vorgänge und Vera-Einstellungen sind nur nach Anmeldung sichtbar.</p>
        {query.error && <p className="error-message" role="alert">Benutzername oder Passwort ist nicht korrekt.</p>}
        <form className="settings-form" action="/api/admin/login" method="post">
          ...
          <label><span>Benutzername</span><input name="username" autoComplete="username" required /></label>
          <label><span>Passwort</span><input name="password" type="password" autoComplete="current-password" required /></label>
          <button className="settings-submit" type="submit">Anmelden</button>
        </form>
      </section>
    </main>
  );
